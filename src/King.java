            ...
            for (int j = column - 1; j <= column + 1; j += 2) {
                if (chessBoard.checkPos(i) && chessBoard.checkPos(j)) {
                    if (chessBoard.board[i][j] != null) {
                        if (!color.equals(chessBoard.board[i][j].getColor()) && chessBoard.board[i][j].getSymbol().equalsIgnoreCase("p"))
                            return true;
                    }
                }
            }
        ...
        for (int i = 0; i < possiblePositions.length; i++) {
            if (chessBoard.checkPos(possiblePositions[i][0]) && chessBoard.checkPos(possiblePositions[i][1])) {
                if (chessBoard.board[possiblePositions[i][0]][possiblePositions[i][1]] != null) {
                    if (!color.equals(chessBoard.board[possiblePositions[i][0]][possiblePositions[i][1]].getColor()) && chessBoard.board[possiblePositions[i][0]][possiblePositions[i][1]].getSymbol().equalsIgnoreCase("h"))
                        return true;
                }
            }
        }
